...
@section('container')
    ...
    <div class="card mb-3 mt-3 text-center">
        ...
        <div class="card-body">
            ...
            <a href="/blog/{{ $posts[0]->slug }}" class="btn btn-danger">Read More</a>
        </div>
    </div>
        ...
        <div class="row">
            ...
            @foreach ($posts->skip(1) as $post)
                ...
                <div class="col-md-4 mb-3">
                    <div class="card">
                        <div class="position-absolute px-3 py-2" style="background-color: rgb(0, 0, 0, 0.7)"><a href="/blog/categories/{{ $post->category->slug }}" class="text-white text-decoration-none">{{ $post->category->name }}</a></div>
                        <img src="https://source.unsplash.com/500x400?{{ $post->category->name }}" class="card-img-top" alt="{{ $post->category->name  }}">
                        <div class="card-body">
                            <a href="/blog/{{ $post->slug }}" class="text-decoration-none text-dark"><h5 class="card-title">{{ $post->title }}</h5></a>
                            <p class="card-text"> 
                                <small class="text-muded">
                            ...
                            <a href="/blog/{{ $post->slug }}" class="btn btn-danger">Read More</a>
                        </div>
                    </div>
                </div>
            @endforeach
    ...
    @else
        <p class="text-center fs-4">No found.</p>
